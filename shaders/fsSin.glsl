precision highp float;
    ...
    void main() {
        ...
        for (int i = 0; i < 2000; i++) {
            ...
            float cos_zx = cos(zx);
            float exp_zy = exp(zy);
            float exp_mzy = exp(-zy);
            float cosh_zy = 0.5 * (exp_zy + exp_mzy);
            float sinh_zy = 0.5 * (exp_zy - exp_mzy);

            float nx = sin_zx * cosh_zy + c_x_scalar;
            float ny = cos_zx * sinh_zy + c_y_scalar;
